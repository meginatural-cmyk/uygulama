...
class AkilliGörüntüleyici:
    ...
    def open_file(self):
        # HTML uzantısı buraya eklendi
        file_path = filedialog.askopenfilename(
            title="Dosya Seçin",
            filetypes=[("Tüm Desteklenenler", "*.pdf *.docx *.xlsx *.png *.jpg *.jpeg *.mp3 *.wav *.mp4 *.txt *.html *.htm")]
        )
        ...
        # --- TARAYICI TABANLI GÖRÜNTÜLEME (HTML, PDF, Video ve Ses) ---
        # .html ve .htm uzantıları buraya eklendi
        if ext in ['.pdf', '.mp4', '.webm', '.mp3', '.wav', '.ogg', '.html', '.htm']:
